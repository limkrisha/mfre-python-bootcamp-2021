...
- `pip` is Python's package manager
  - Need to frontend with `!` to execute a shell/cmd command 
...
```{python, eval = F}
df = pd.read_csv("myanmar-supermarket.csv")
df.head(5)
```
...
```{python, eval = F}
df2 = pd.read_csv("/content/drive/MyDrive/myanmar-supermarket.csv")
df2.head(5)
```
...
df3 = pd.read_csv(dir + "/MyDrive/myanmar-supermarket.csv")
df3.head(5)
```

- If all your files are in the `/MyDrive/` extension, you can also add that to the object `dir`
...
df4 = pd.read_csv(dir + "/myanmar-supermarket.csv")
df4.head(5)
```
...
# change the directorty to MyDrive
...
```{python, eval = F}
import pandas as pd
supermarket = pd.read_csv("C:\Users\krishlim\OneDrive - UBC\Desktop\mfre\2021w-python-bootcamp\data")
```
